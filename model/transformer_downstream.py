...
class ActionHeadLinprobe(nn.Module):
    def __init__(self, dropout_ratio=0., dim_feat=512, num_classes=60, num_joints=25):
        super(ActionHeadLinprobe, self).__init__()
        self.fc = nn.Linear(dim_feat, num_classes)
        
    def forward(self, feat):
        '''
            Input: (N, M, T, J, C) or (N, C)
        '''
        if len(feat.shape) == 5:
...
class ActionHeadLinprobe2(nn.Module):
    def __init__(self, dropout_ratio=0., dim_feat=512, num_classes=60, num_joints=25):
        super(ActionHeadLinprobe2, self).__init__()
        self.dropout = nn.Dropout(p=dropout_ratio)
        self.fc = nn.Linear(dim_feat*num_joints, num_classes)
    ...
    def forward(self, feat):
        ...
        feat = feat.mean(dim=[1,2]).reshape(N, J*C)
        feat = self.fc(feat)
        return feat
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        ### change scale ###
        #self.scale = (head_dim ** -0.5) * np.log(750) / np.log(75)
...
class Transformer(nn.Module):
    def __init__(self, dim_in=3, num_classes=3, dim_feat=256,
        ...
        if protocol == 'linprobe':
            self.head = ActionHeadLinprobe(dropout_ratio=0., dim_feat=dim_feat, num_classes=num_classes)
        elif protocol == 'linprobe2':
            self.head = ActionHeadLinprobe2(dropout_ratio=0.3, dim_feat=dim_feat, num_classes=num_classes)
    ...
    def forward(self, x, feature_only=False):
        ...
        #print(x.std(dim=[0,1,2], keepdim=True))
        #x = x / x.std(dim=[0,1,2], keepdim=True)
