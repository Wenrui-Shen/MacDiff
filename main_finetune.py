...
#from engine_finetune_2dto3d import train_one_epoch_2dto3d, evaluate_2dto3d

from data.h36m.datareader_h36m import DataReaderH36M
...
def get_args_parser():
    ...
    parser.add_argument('--head_only_epochs', default=0, type=int, metavar='N', help='head only epoch')
    parser.add_argument('--head_only', type=bool, default=False, help='')
    parser.add_argument('--warmup_epochs', type=int, default=5, metavar='N', help='epochs to warmup LR')
    parser.add_argument('--min_lr_epochs', type=int, default=10, metavar='N', help='epochs to keep min LR at the end of training')
    ...
    parser.add_argument('--semi', type=float, default=1., help='')
...
def main(args):
    ...
    print(dataset_val)
    if args.task == '2dto3d':
        datareader = DataReaderH36M(n_frames=128, sample_stride=1, data_stride_train=32, data_stride_test=128, dt_file = 'h36m_sh_conf_cam_source_final.pkl', dt_root='data/h36m/')
    ...
    if args.distributed:
        ...
        if args.dist_eval:
            assert args.task != '2dto3d', "3D Reconstruction task do NOT use dist_eval"
    ...
    if not args.head_only:
        ...
        optimizer = torch.optim.AdamW(model_without_ddp.head.parameters(), lr=args.lr)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.eval_epoch != 0 and epoch < args.epochs-10: 
            test_stats = {}
