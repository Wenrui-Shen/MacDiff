...
def get_args_parser():
    ...
    parser.add_argument('--lambda_skeleton_to_text', type=float, default=None,
                        help='Override the fixed skeleton-to-text loss weight')
...
def main(args):
    ...
    if args.text_cache and args.enable_ose:
        raise ValueError('Global text Stage1 and OSE cannot be enabled together')
    if args.enable_ose:
    ...
    supports_text = getattr(Model, 'supports_text_cache', False)
    if supports_text != bool(args.text_cache):
        raise ValueError('Use transformer_macdiff_text.Transformer together with --text_cache')
    model_args = dict(args.model_args)
    for name in ('lambda_text_to_skeleton', 'lambda_skeleton_to_text'):
