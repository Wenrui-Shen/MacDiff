...
from data.h36m.tools import read_pkl
from data.h36m.utils_data import flip_data
    
class MotionDataset(Dataset):
    def __init__(self, data_root, data_split): # data_split: train/test
        np.random.seed(0)
        self.data_root = data_root #args.data_root
        self.data_split = data_split
        file_list_all = []
        data_path = os.path.join(self.data_root, self.data_split)
...
class MotionDataset3D(MotionDataset):
    def __init__(self, data_root, data_split, flip=True, synthetic=False, gt_2d=False):
        super(MotionDataset3D, self).__init__(data_root, data_split)
        self.flip = flip #args.flip
        self.synthetic = synthetic #args.synthetic
        #self.aug = Augmenter3D(args)
        self.gt_2d = gt_2d #args.gt_2d
