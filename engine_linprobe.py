...
from util.class_acc import class_accuracy_meter
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    if args.motion_aware_tau > 0.:
        ...
        print('Use (spatial) temporal pooling')
    ...
    for data_iter_step, (samples, samples_aug, targets, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast(enabled=args.enable_amp):
            ...
            if args.motion_aware_tau > 0:
                outputs = model(samples, args.motion_aware_tau)
            else:
...
def evaluate(data_loader, model, device):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        with torch.no_grad():
            ...
            cls_meter.update(output, target) ###
    ...
    cls_meter.print_class_accuracy()
...
@torch.no_grad()
def evaluate_original(data_loader, model, device):
    criterion = torch.nn.CrossEntropyLoss()
...
class LabelSmoothCELoss(torch.nn.Module):
    ...
    def __init__(self, label_smooth=0.):
        super().__init__()
        self.label_smooth = label_smooth
        print(f'Using LabelSmoothCELoss, label_smooth = {label_smooth}')
        assert label_smooth > 0. and label_smooth < 1., f'Wrong label_smooth {label_smooth}'
    ...
    def forward(self, pred, target):
        ...
        target = F.one_hot(target, class_num)	# turn into one-hot          
        
        target = torch.clamp(target.float(), min=self.label_smooth/(class_num-1), max=(1.-self.label_smooth))
        loss = - torch.sum(target * logprobs, 1)
